...
object CalculerHoraire3 extends App with jacop{
  
    /*
     * renvoie les series
     * 
     */
  def getSeries: List[String] = { List("Serie 01", "Serie 02") }
  
  
    /*
     * renvoie l'horaire de toutes les series
     * 
     */
  ...
  def getResultat: List[Map[String, List[List[String]]]] = {
    ...
    /*
     * contrainte: Le prof ne peut pas etre à 2 endroits à la fois, Pas 2 cours dans le meme local
     */
    for (i <- List.range(0, iProfesseurs.length / series.length)) {

      // Grolaux ne commence jamais à lundi
      //	  if( i==0 || i%(5-1)==0){
